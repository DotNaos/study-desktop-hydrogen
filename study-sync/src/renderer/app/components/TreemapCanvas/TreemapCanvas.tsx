...
export function TreemapCanvas({
    ...
    // Collect completed IDs from data
    useEffect(() => {
        const ids = new Set<string>()
        const scan = (n: Node) => {
            if (n.isComplete) ids.add(n.id)
    ...
    const handleNodeClick = (node: Node, e: React.MouseEvent) => {
        ...
        if (node.type === 'item' || isLeaf) {
            // Navigate to task - use node.id as fallback for missing courseId/taskId
            const courseId = node.courseId || node.id
    ...
    // Toggle completion status for a leaf node
    const toggleCompletion = (id: string) => {
        ...
        // TODO: Call API to persist completion status
    }
    ...
    const getProgress = (node: Node): number => {
        ...
        if (isLeaf) {
            ...
            return node.isComplete || completedIds.has(node.id) ? 100 : 0
        }
        ...
        const computeFromChildren = (n: Node) => {
            ...
            if (nIsLeaf) {
                ...
                } else if (n.isComplete || completedIds.has(n.id)) {
                    totalProgress += 100
                }
            } else {
            ...
            <div className="flex-1 flex items-center justify-center p-4">
                ...
                >
                    {children.map((item) => {
                        ...
                        const isComplete = item.isComplete || completedIds.has(item.id)
                        ...
                        // Wrap leaf nodes with context menu for completion toggle
                        if (isLeaf) {
                            return (
